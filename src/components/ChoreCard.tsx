import { Calendar, Check, Clock, MoreVertical, Play, RotateCcw } from 'lucide-react';
import {
...
export function ChoreCard({
  ...
  const pillLabel = completed
    ? 'DONE'
    : intervalLabel === 'One-off'
      ? 'ONE-TIME'
      : intervalLabel.toUpperCase();
  const dueToday = status.kind === 'today';
  const showDueMeta =
    !completed && status.kind !== 'someday' && Boolean(chore.next_due_at);
  const completedLabel = chore.last_completed_at
    ? `Completed ${formatChoreShortDate(chore.last_completed_at)}`
    : 'Completed';
    ...
    <div data-task-interactive="" className={cardClassName}>
      <div className="flex items-start justify-between gap-3">
        <button
          type="button"
          onClick={() =>
            onOpenDetail ? onOpenDetail(chore.id) : completed ? undefined : onStart(chore.id)
          }
          className="min-w-0 flex-1 text-left"
        >
          <p
            className={`line-clamp-3 text-[18px] font-bold leading-snug ${
              completed
      ...
      {completed ? (
        <div className="flex flex-wrap items-center gap-1.5 text-[13px] font-medium text-[#9999a1]">
